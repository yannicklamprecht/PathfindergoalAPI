...
public class PathfinderGoalSteerEntity implements PathfinderGoal {
  ...
  public PathfinderGoalSteerEntity(PathfinderManager pathfinderManager,
      Insentient pathfinderGoalEntity) {
  ...
  public void init() {
    ...
    if (pathfinderPlayer == null) {
      getSteeringPlayer().ifPresent(
          player -> this.pathfinderPlayer = pathfinderManager.getPathfinderPlayer(player));
    }

  }

  private Optional<Player> getSteeringPlayer() {
    return pathfinderGoalEntity.getBukkitEntity().getPassengers().stream()
        .filter(Player.class::isInstance).map(Player.class::cast).findAny();
  }
  ...
  public void execute() {
    ...
    Location blockInDirection = player.getLocation()
        .add(player.getLocation().getDirection().setY(0).normalize().multiply(1));

    if (blockInDirection.getBlock().getType().isSolid() && !blockInDirection.add(0, 1, 0).getBlock()
        .getType().isSolid()) {
