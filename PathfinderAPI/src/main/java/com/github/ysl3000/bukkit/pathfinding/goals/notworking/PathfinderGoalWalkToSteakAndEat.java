...
public class PathfinderGoalWalkToSteakAndEat implements PathfinderGoal {
  ...
  @Override
  public void init() {
    Optional<Entity> nearestStack = pathfinderGoalEntity.getBukkitEntity()
        .getNearbyEntities(distance, distance, distance).stream()
        .filter(e -> e.getType() == EntityType.DROPPED_ITEM).min(distanceComperator);
    ...
    if (this.target != null) {
      if (this.target instanceof Item) {
        ...
        if (buEntity instanceof LivingEntity) {
          ((LivingEntity) buEntity).getEquipment()
              .setItemInMainHand(((Item) this.target).getItemStack());
        }
        this.target.remove();
        this.target = null;
      }
  ...
  private class DistanceComperator implements Comparator<Entity> {
    ...
    private double getDistance(Entity entity) {
      return entity.getLocation()
          .distanceSquared(pathfinderGoalEntity.getBukkitEntity().getLocation());
    }
